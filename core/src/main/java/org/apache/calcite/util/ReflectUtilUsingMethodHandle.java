 ...
 */
public abstract class ReflectUtilUsingMethodHandle {
   ...
   */
  public static MethodHandle lookupVisitMethod(
      Class<?> visitorClass,
      Class<?> visiteeClass,
  ...
  public static MethodHandle lookupVisitMethod(
    ...
    Map<Class<?>, MethodHandle> cache = new HashMap<>();
  ...
  private static MethodHandle lookupVisitMethod(
    ...
    for (Class<?> anInterface : interfaces) {
      final MethodHandle method = lookupVisitMethod(visitorClass, anInterface,
   ...
   */
  public static <R extends ReflectiveVisitor, E> ReflectiveVisitDispatcher2<R, E> createDispatcher(
    ...
    return new ReflectiveVisitDispatcher2<R, E>() {
      final Map<List<Object>, MethodHandle> map = new HashMap<>();

      public MethodHandle lookupVisitMethod(
      ...
      public MethodHandle lookupVisitMethod(
        ...
        MethodHandle method = map.get(key);
        if (method == null) {
          if (map.containsKey(key)) {
            // We already looked for the method and found nothing.
          } else {
            method =
                ReflectUtilUsingMethodHandle.lookupVisitMethod(
                    visitorClass,
                    visiteeClass,
                    visitMethodName,
                    returnType,
  ...
  public static <E, T> MethodDispatcher<T> createMethodDispatcher(
    ...
    final ReflectiveVisitDispatcher2<ReflectiveVisitor, E>
        dispatcher =
        createDispatcher(
            (Class<ReflectiveVisitor>) visitor.getClass(), arg0Clazz);
    return new MethodDispatcher<T>() {
      public T invoke(Object... args) {
        MethodHandle method = lookupMethod(args[0]);
        try {
      ...
      private MethodHandle lookupMethod(final Object arg0) {
